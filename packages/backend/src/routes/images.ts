...
import { uploadImagesSchema, listImagesSchema, imageDetailSchema, imageIdSchema, deleteImageSchema, batchDeleteImagesSchema } from '../validators/imageValidators';
...
const storage = multer.diskStorage({
    destination: function (req: MulterRequest, _file: Express.Multer.File, cb: (error: Error | null, destination: string) => void) {
        ...
        if (projectId.startsWith('project-')) {
            projectId = projectId.substring(8); // Remove "project-" prefix for storage path
            logger.debug('Removed project- prefix for storage path', {
                originalId: req.params.projectId,
                cleanedId: projectId
            });
        }
        ...
        if (!fs.existsSync(projectUploadDir)) {
            ...
            logger.debug('Created project upload directory', { path: projectUploadDir });
        }
        cb(null, projectUploadDir);
    },
    filename: function (_req: MulterRequest, file: Express.Multer.File, cb: (error: Error | null, filename: string) => void) {
...
async function processAndStoreImage(file: Express.Multer.File, projectId: string, userId: string, client: PoolClient): Promise<ImageData> {
    ...
    try {
        ...
        if (!fileExists) {
            logger.error('Uploaded file missing from filesystem', { path: file.path });
        ...
        // Generate thumbnail
        await imageUtils.createThumbnail(file.path, thumbnailPath, { width: 200, height: 200 });
        ...
        for (const filePath of cleanupPaths) {
            try {
                ...
                logger.debug('Cleaned up file after processing error', { path: filePath });
            } catch (deleteError) {
        ...
        throw new ApiError(`Failed to process image: ${file.originalname} - ${processError.message || 'Unknown error'}`, 500);
    }
...
router.post(
    ...
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        ...
        if (!requestingUserId || typeof requestingUserId !== 'string') {
           logger.error('User ID not found in authenticated request', { userId: requestingUserId });
           throw new ApiError('User ID not found in authenticated request', 500);
        }

        // Log user ID for debugging
        logger.debug('Processing image upload request', { userId: requestingUserId });
        ...
        try {
            ...
            try {
                ...
                if (storageInfoRes.rows.length === 0) {
                    logger.warn('User not found for storage check, using defaults', { userId: requestingUserId });
                } else {
                ...
                if (files) {
                    files.forEach(file => {
                        fs.unlink(file.path, (err) => {
                            if (err) logger.error('Failed to clean up file after storage limit exceeded', { path: file.path, error: err });
                        });
                    });
                }
            ...
            const projectCheck = await pool.query('SELECT id FROM projects WHERE id = $1 AND user_id = $2', [projectId, requestingUserId]);
            ...
            try {
                ...
                const successfullyUploadedSize = insertedImages.reduce((sum, img) => sum + BigInt(img.size || 0), 0n);
            ...
            allUploadedFilePaths.forEach(filePath => {
                if (fs.existsSync(filePath)) {
                    try {
                        ...
                        logger.debug('Cleaned up file after global error', { path: filePath });
                    } catch (unlinkErr) {
 ...
 */
router.delete('/projects/:projectId/images/:imageId', authMiddleware, validate(deleteImageSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!result.success) {
            if (result.error?.includes('not found') || result.error?.includes('access denied')) {
                logger.warn('Image deletion denied', { projectId, originalProjectId, imageId, error: result.error });
                return res.status(404).json({ message: result.error });
            }
            logger.error('Failed to delete image', { projectId, originalProjectId, imageId, error: result.error });
...
router.delete('/images/:id', authMiddleware, validate(imageIdSchema), async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user?.userId;
    ...
    try {
        ...
        if (!result.success) {
            logger.error('Deprecated route: Failed to delete image', { imageId, error: result.error });
 ...
 */
router.get('/projects/:projectId/images/:imageId', authMiddleware, validate(imageDetailSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const userId = req.user?.userId;
    let { projectId } = req.params;
    const imageId = req.params.imageId;
...
router.get('/projects/:projectId/images', authMiddleware, validate(listImagesSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (verifyFiles) {
            logger.debug('Verifying file existence for images', { count: processedImages.length });
            ...
            if (filterMissing) {
                ...
                logger.debug('Images after filtering', { count: processedImages.length });
            }
        }
 ...
 */
router.get('/projects/:projectId/images/:imageId/verify', authMiddleware, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const userId = req.user?.userId;
    let { projectId } = req.params;
    const imageId = req.params.imageId;
...
// DELETE /api/projects/:projectId/images/:imageId - Delete an image by ID in a project
// @ts-ignore
router.delete('/projects/:projectId/images/:imageId', authMiddleware, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const userId = req.user?.userId;
    let { projectId } = req.params;
    ...
    try {
        ...
        if (projectCheck.rows.length === 0) {
            logger.warn('Project not found or access denied during image deletion', { projectId, originalProjectId, userId });
            return res.status(404).json({ message: 'Project not found or access denied' });
        }

        // Now verify the image exists in this project
        ...
        if (imageCheck.rows.length === 0) {
            logger.warn('Image not found in project during deletion', { imageId, projectId });
            return res.status(404).json({ message: 'Image not found in this project' });
        }

        // Delete segmentation data if exists
        try {
            await pool.query('DELETE FROM segmentation_results WHERE image_id = $1', [imageId]);
            logger.info('Deleted segmentation results for image', { imageId });
        } catch (segmentationError) {
            // Log but continue with deletion even if segmentation delete fails
            logger.error('Error deleting segmentation results', { error: segmentationError, imageId });
        }

        // Delete the image from the database
        const deleteResult = await pool.query(
            'DELETE FROM images WHERE id = $1 AND project_id = $2 RETURNING id',
            [imageId, projectId]
        );

        if (deleteResult.rowCount === 0) {
            logger.warn('Image not deleted from database', { imageId, projectId });
            return res.status(500).json({ message: 'Failed to delete image' });
        }

        // Return success
        logger.info('Image deleted successfully', { imageId, projectId });
        res.status(200).json({ message: 'Image deleted successfully' });
    } catch (error) {
        logger.error('Error deleting image', { error, imageId, projectId });
        next(error);
    }
});
