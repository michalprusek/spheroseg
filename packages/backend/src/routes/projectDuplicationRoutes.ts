...
router.post('/', authMiddleware, validate(duplicateProjectSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    logger.info('Processing project duplication request', { userId, originalProjectId, newTitle });
...
router.get('/', authMiddleware, validate(getDuplicationsSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const userId = req.user?.userId;
  const { limit = 10, offset = 0 } = req.query as unknown as { limit: number; offset: number };

  if (!userId) return res.status(401).json({ message: 'Authentication error' });

  try {
    logger.info('Fetching duplication tasks for user', { userId, limit, offset });
...
router.get('/:taskId', authMiddleware, validate(getDuplicationSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const userId = req.user?.userId;
  ...
  try {
    ...
    if (!task) {
      logger.info('Duplication task not found or not authorized', { userId, taskId });
    ...
    logger.error('Error fetching duplication task', { error, userId, taskId });
    next(error);
  }
});

// DELETE /api/duplication/:taskId - Cancel a duplication task
router.delete('/:taskId', authMiddleware, validate(cancelDuplicationSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const userId = req.user?.userId;
  const { taskId } = req.params;
  ...
  try {
    ...
    logger.error('Error cancelling duplication task', { error, userId, taskId });
    next(error);
  }
});
