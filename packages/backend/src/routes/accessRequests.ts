...
router.post('/', validate(createAccessRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        try {
            logger.info('Sending access request email notification', { email, name });
            ...
            logger.error('Failed to send access request email notification', { error: emailError, email, name });
