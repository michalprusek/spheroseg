...
describe('Logs API Controller', () => {
  ...
  describe('getLogs', () => {
      ...
      const mockLogs = {
        logs: [
          ...
          },
          {
            ...
            metadata: { query: 'SELECT * FROM users', error: 'Connection timeout' }
          }
        ],
        total: 2,
        page: 1,
    ...
    it('handles server errors when fetching logs', async () => {
      ...
      vi.mocked(loggingService.getLogs).mockRejectedValue(
  ...
  describe('getLogDetails', () => {
    ...
    it('handles log not found errors', async () => {
      ...
      vi.mocked(loggingService.getLogById).mockRejectedValue(
        new ApiError(404, 'Log not found')
      );
