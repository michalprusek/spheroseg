...
jest.mock('../../middleware/validationMiddleware', () => ({ // Mock validation middleware
...
describe('Auth Routes - /api/auth', () => {
    ...
    describe('POST /signup', () => {
        it('should register a new user successfully and return 201', async () => {
            const newUserDbResponse = { id: userId, email: testEmail, is_approved: false, role: 'user' };
            mockPoolQuery
        ...
        it('should return 409 if email is already registered', async () => {
            mockPoolQuery.mockResolvedValueOnce({ rows: [{ id: 'existing-user-id' }], rowCount: 1 }); // User check -> Found
    ...
    describe('POST /login', () => {
        ...
        it('should log in an approved user successfully and return 200 with token', async () => {
            mockPoolQuery.mockResolvedValueOnce({ rows: [userDbRecord], rowCount: 1 }); // User found
            ...
            expect(mockJwtSign).toHaveBeenCalledWith({ userId: userId, email: testEmail }, process.env.JWT_SECRET, { expiresIn: '1h' });
        });
        ...
        it('should return 403 if user is not approved', async () => {
            ...
            mockPoolQuery.mockResolvedValueOnce({ rows: [unapprovedUser], rowCount: 1 }); // User found, not approved
            mockBcryptCompare.mockResolvedValueOnce(true); // Password matches
