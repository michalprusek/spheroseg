...
// Message types
type WorkerMessage = 
  | { type: 'progress'; progress: number; processedItems: number; totalItems: number }
  | { type: 'complete'; result: any; processedItems: number; totalItems: number }
  | { type: 'error'; error: string; progress?: number; processedItems?: number; totalItems?: number };
 ...
 */
function sendComplete(result: any, processedItems: number, totalItems: number): void {
  sendMessage({
    type: 'complete',
    result,
    processedItems,
...
async function processBatch(
  ...
  for (const image of batch) {
    try {
      ...
      console.error(`Error preparing image duplication for ${image.id}:`, imageError);
  ...
  for (const op of dbOperations) {
    try {
      ...
      if (processedItems % 5 === 0 || progress >= 100) { // Don't send too many updates
        sendProgress(progress, processedItems, totalItems);
      }
    } catch (dbError) {
      console.error('Error executing database operation:', dbError);
...
async function processProjectDuplication(): Promise<void> {
  try {
    ...
    console.log(`Starting project duplication task ${taskId}`);
...
if (parentPort) {
  parentPort.on('message', (message) => {
    if (message === 'cancel') {
      console.log('Received cancel message, shutting down worker');
      process.exit(0);
    }
  });
}
