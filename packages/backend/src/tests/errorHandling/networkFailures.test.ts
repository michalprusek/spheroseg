...
describe('Network Failure Tests', () => {
  ...
  async function testNetworkFailure(endpoint: string): Promise<any> {
    try {
  ...
  it('should have proper retry and circuit breaker in segmentation service', async () => {
    ...
    jest.mock('../../services/segmentationService', () => {
      ...
      return {
        ...
        _callMlService: jest.fn().mockImplementation(async (endpoint, data) => {
          try {
            const response = await fetch(`${mockServerUrl}${endpoint}`);
            if (!response.ok) {
    ...
    try {
      await segmentationService._callMlService('/retry-success', {});
