...
describe('Segmentation Version Management', () => {
  ...
  it('should retrieve segmentation history versions', async () => {
    const response = await request(app)
      .get(`/api/images/${testImageId}/segmentation/history`);
  ...
  it('should support pagination for history versions', async () => {
    const response = await request(app)
      .get(`/api/images/${testImageId}/segmentation/history?limit=2&offset=1`);
  ...
  it('should retrieve a specific version of segmentation data', async () => {
    const version = 3;
    const response = await request(app)
      .get(`/api/images/${testImageId}/segmentation/history/${version}`);
    ...
    expect(response.body.result_data.polygons[0].points[0]).toEqual({ x: 10 * version, y: 10 * version });
  });
  ...
  it('should restore a previous version to current', async () => {
    ...
    const currentDataResponse = await request(app)
      .get(`/api/images/${testImageId}/segmentation`);
  ...
  it('should handle restoring a non-existent version', async () => {
    ...
    db.query = vi.fn().mockImplementation((query, params) => {
      if (query.includes('SELECT * FROM segmentation_history WHERE image_id = $1 AND version = $2') && params[1] === nonExistentVersion) {
        return { rows: [] };
      }
  ...
  it('should compare two versions of segmentation data', async () => {
    ...
    const response = await request(app)
      .get(`/api/images/${testImageId}/segmentation/history/compare?version1=${version1}&version2=${version2}`);
  ...
  it('should list snapshots separately from regular versions', async () => {
    const response = await request(app)
      .get(`/api/images/${testImageId}/segmentation/snapshots`);
  ...
  it('should list all branches for an image', async () => {
    const response = await request(app)
      .get(`/api/images/${testImageId}/segmentation/branches`);
...
describe('Project-level Segmentation Version Management', () => {
  ...
  it('should get versions for all segmentations in a project', async () => {
    const response = await request(app)
      .get(`/api/projects/${testProjectId}/segmentations/history`);
