...
describe('Project Creation API', () => {
  ...
  describe('POST /projects', () => {
    it('should create a new project', async () => {
      ...
      expect(mockDbQuery).toHaveBeenCalledWith(
        ...
        ['test-user-id', 'Test Project', 'Test Description']
      );
    });
  ...
  describe('POST /projects/:id/duplicate', () => {
    it('should duplicate a project', async () => {
      const response = await request(app)
        .post('/projects/test-project-id/duplicate')
        .send({
    ...
    it('should return 404 if project does not exist', async () => {
      ...
      const response = await request(app)
        .post('/projects/non-existent-project/duplicate')
        .send({
          newTitle: 'Duplicated Project'
        });
