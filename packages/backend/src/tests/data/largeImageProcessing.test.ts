...
describe('Large Image Processing Tests', () => {
  ...
  it('should handle large images without excessive memory usage', async () => {
    ...
    console.log(`Memory usage increase - Heap: ${heapUsageIncrease / 1024 / 1024}MB, RSS: ${rssIncrease / 1024 / 1024}MB`);
  ...
  async function processComplexPolygons(polygons: any[]) {
    ...
    return {
      ...
      simplifiedPolygons,
      compressionRatio: simplifiedPolygons.reduce((sum, p) => sum + p.vertices.length, 0) / 
