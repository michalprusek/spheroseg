...
export const socketAuthMiddleware = (socket: Socket, next: (err?: Error) => void) => {
  try {
    ...
    if (config.isDevelopment) {
      if (!token) {
        logger.info('Development mode: Using mock user ID for WebSocket due to missing token');
        socket.data.user = {
      ...
      try {
        ...
        logger.warn('Token verification failed in development mode, using mock user', { 
