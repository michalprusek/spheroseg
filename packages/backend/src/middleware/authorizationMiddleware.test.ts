...
describe('Authorization Middleware', () => {
    ...
    describe('isAdmin', () => {
        it('should call next() if user is admin', async () => {
            ...
            mockDbQuery.mockResolvedValueOnce({ rows: [{ role: 'admin' }], rowCount: 1 }); // Mock DB response
        ...
        it('should return 403 if user is not admin', async () => {
            ...
            mockDbQuery.mockResolvedValueOnce({ rows: [{ role: 'user' }], rowCount: 1 });

            await isAdmin(mockRequest as AuthenticatedRequest, response, mockNext);

            expect(mockDbQuery).toHaveBeenCalledWith('SELECT role FROM users WHERE id = $1', ['regular-user-id']);
            expect(response.status).toHaveBeenCalledWith(403);
            expect(response.json).toHaveBeenCalledWith({ message: 'Forbidden: Admin access required' });
    ...
    describe('isUserApproved', () => {
        it('should call next() if user is approved', async () => {
            mockRequest.user = { userId: 'approved-user-id', email: 'approved@test.com' };
            mockDbQuery.mockResolvedValueOnce({ rows: [{ is_approved: true }], rowCount: 1 });
        ...
        it('should return 403 if user is not approved', async () => {
            mockRequest.user = { userId: 'not-approved-user-id', email: 'notapproved@test.com' };
            mockDbQuery.mockResolvedValueOnce({ rows: [{ is_approved: false }], rowCount: 1 });

            await isUserApproved(mockRequest as AuthenticatedRequest, response, mockNext);

            expect(mockDbQuery).toHaveBeenCalledWith('SELECT is_approved FROM users WHERE id = $1', ['not-approved-user-id']);
            expect(response.status).toHaveBeenCalledWith(403);
            expect(response.json).toHaveBeenCalledWith({ message: 'Forbidden: Account not approved' });
            expect(mockNext).not.toHaveBeenCalled();
        });
