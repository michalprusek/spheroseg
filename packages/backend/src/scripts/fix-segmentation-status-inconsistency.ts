  ...
  try {
    ...
    for (const image of inconsistentImages) {
      try {
        ...
        if (image.has_result_data || image.segmentation_result_status === 'completed' || image.task_status === 'completed') {
          targetStatus = SEGMENTATION_STATUS.COMPLETED;
          reason = 'Has result data or completion status in other tables';
        } else if (image.task_status === 'failed' || image.segmentation_result_status === 'failed') {
          targetStatus = SEGMENTATION_STATUS.FAILED;
          reason = 'Failed status in other tables';
        } else if (image.result_updated_at && new Date(image.result_updated_at) < new Date(Date.now() - 3600000)) {
        ...
        if (!dryRun) {
          ...
          if (image.task_status && image.task_status !== 'completed' && image.task_status !== 'failed') {
            await client.query(
    ...
    const summaryResult = await pool.query(summaryQuery, [inconsistentImages.map(img => img.image_id)]);
