...
export async function deleteImage(
  ...
  let result: DeleteImageResult = { imageId, success: false };
  ...
  try {
    ...
    const imageCheck = await client.query(
      `SELECT i.id, i.storage_path, i.thumbnail_path, i.size 
    ...
    imageData = {
      imageId,
      storageSize: BigInt(imageCheck.rows[0].size || 0),
    ...
    if (imageData.storagePath) {
      try {
        ...
        logger.warn('Error checking/removing empty project directory', { error });
    ...
    try {
      ...
      logger.error('Image deletion transaction rolled back', { imageId, error });
    } catch (rollbackError) {
