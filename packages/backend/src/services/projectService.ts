...
export async function createProject(
  ...
  const { title, description, userId, tags = [], public: isPublic = false } = params;
  ...
  try {
    ...
    try {
      ...
      logger.error('Failed to create project directories', { error, projectId: newProject.id });
...
export async function getProjectById(
  ...
  try {
    ...
    logger.debug('Error checking for shared project', { error, projectId, userId });
  }
...
export async function getUserProjects(
  ...
  if (includeShared) {
    try {
      ...
      logger.debug('Error checking for project_shares table for counting', { error });
    }
  }
  ...
  try {
    ...
    try {
      ...
      if (projectExists.rows.length === 0) {
        ...
        logger.info('Project not found during deletion attempt', { projectId, userId });
      ...
      logger.error('Database error checking project ownership', { error: err, projectId, userId });
    ...
    try {
      ...
      if (fs.existsSync(projectDir)) {
        ...
        logger.info('Project directory not found during deletion', { projectId, projectDir });
      }
    } catch (error) {
