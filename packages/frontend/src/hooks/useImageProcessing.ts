...
export function useImageProcessing(options: UseImageProcessingOptions = {}) {
  ...
  // Load image utility
  const loadImage = useCallback(async (source: string | File | Blob) => {
    try {
      setState(prev => ({
        ...prev,
        isProcessing: true,
...
export function useImageProcessingPresets() {
  ...
  const presets = {
    ...
    socialMedia: {
      instagram: {
        square: async (source: File | Blob) => 
          resizeImage(source, { maxWidth: 1080, maxHeight: 1080 }),
        story: async (source: File | Blob) => 
          resizeImage(source, { maxWidth: 1080, maxHeight: 1920 }),
      },
      twitter: async (source: File | Blob) => 
        resizeImage(source, { maxWidth: 1200, maxHeight: 675 }),
      facebook: async (source: File | Blob) => 
        resizeImage(source, { maxWidth: 1200, maxHeight: 630 }),
    },
