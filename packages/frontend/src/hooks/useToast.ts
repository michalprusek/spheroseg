...
export function useToast() {
  ...
  const loadingSuccess = useCallback(
    (idOrMessage: string | number, message: string, options?: ToastOptions) => {
      let id = idOrMessage;
  ...
  const confirm = useCallback(
    (message: string, onConfirm: () => void, onCancel?: () => void, options?: ToastOptions) => {
      return toastService.confirm(translateMessage(message), onConfirm, onCancel, options);
    },
    [translateMessage],
  );
  ...
  const multiline = useCallback(
    (title: string, description: string, type: ToastType = 'info', options?: ToastOptions) => {
      return toastService.multiline(translateMessage(title), translateMessage(description), type, options);
    },
    [translateMessage],
  );
  ...
  const progress = useCallback(
    (message: string, progress: number, options?: ToastOptions) => {
