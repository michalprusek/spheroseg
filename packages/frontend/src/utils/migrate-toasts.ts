...
const MIGRATION_RULES: MigrationRule[] = [
  ...
  {
    pattern: /import\s+{\s*showSuccess,\s*showError,\s*showInfo,\s*showWarning\s*}\s+from\s+['"]@\/utils\/toastUtils['"]/g,
    replacement: "import { useToast } from '@/hooks/useToast'"
  },
  {
...
function migrateFile(filePath: string): boolean {
  ...
  if (!content.includes('toast') && !content.includes('showSuccess') && 
      !content.includes('showError') && !content.includes('showInfo') && 
  ...
  if (isComponent) {
    ...
    if (content.includes('toast.') || content.includes('showSuccess') || 
      ...
      if (componentMatch && !content.includes('const toast = useToast()') && 
    ...
    // Replace old utility functions
    if (content.includes('showSuccess') || content.includes('showError') || 
        content.includes('showInfo') || content.includes('showWarning')) {
      content = content.replace(
