...
export const api = {
  ...
  users: {
    ...
    uploadAvatar: (file: File) => 
      apiClient.upload<{ url: string }>('/users/me/avatar', file),
  ...
  projects: {
    ...
    unshare: (id: string, userId: string) => 
      apiClient.delete(`/projects/${id}/share/${userId}`),
  },
  ...
  images: {
    ...
    update: (id: string, data: Partial<Image>) => 
      apiClient.patch<Image>(`/images/${id}`, data),
    
    delete: (id: string) => 
      apiClient.delete(`/images/${id}`),
    
    deleteMany: (ids: string[]) => 
      apiClient.post('/images/delete-many', { ids }),
    
    segment: (id: string) => 
      apiClient.post<{ taskId: string }>(`/images/${id}/segment`),
    
    segmentMany: (ids: string[]) => 
      apiClient.post<{ taskId: string }>('/images/segment-many', { ids }),
  },

  // Segmentation
  segmentation: {
    getSegments: (imageId: string) => 
      apiClient.get<Segment[]>(`/segmentation/segments/${imageId}`),
    ...
    deleteSegment: (id: string) => 
      apiClient.delete(`/segmentation/segments/${id}`),
  ...
  // Metadata
  metadata: {
    update: (id: string, metadata: Record<string, any>) => 
      apiClient.patch(`/metadata/${id}`, metadata),
    ...
    enrich: (ids: string[], provider: 'openai' | 'custom') => 
      apiClient.post('/metadata/enrich', { ids, provider }),
  ...
  stats: {
    ...
    project: (projectId: string) => 
      apiClient.get<Record<string, any>>(`/stats/projects/${projectId}`),
    
    user: () => 
      apiClient.get<Record<string, any>>('/stats/user'),
  },
};
