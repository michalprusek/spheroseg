...
const CanvasV2: React.FC<CanvasV2Props> = ({
    ...
    const calculateCursorPosition = useCallback((e: React.MouseEvent<HTMLDivElement>): Point | null => {
    ...
    return (
        <div
            ...
            style={{ width: '100%', height: '100%', overflow: 'hidden', background: '#222', cursor: 'default', position: 'relative' /* Default cursor */}}
            onMouseDown={(e) => {
        ...
        >
            ...
            <svg
                ...
                <g transform={transformString}>
                    ...
                    }, [segmentationData?.polygons, transform, selectedPolygonId, canvasRef.current?.clientWidth, canvasRef.current?.clientHeight])}

                    {/* Render Vertices for Selected Polygon in edit vertices mode */}
                    {selectedPolygonId && editMode !== EditMode.Slice && editMode !== EditMode.AddPoints && (
                        (() => {
                                ...
                                return (
                                    <circle
                                        ...
                                        onClick={(e) => {
                                            ...
                                            if (isShiftPressed) {
                                                e.stopPropagation();
                                                console.log(`[CanvasV2] Vertex clicked in Edit mode with Shift: polygon=${selectedPolygonId}, vertex=${index}`);
                        ...
                        <>
                            {segmentationData.polygons.map(polygon => (
                                polygon.points.map((point, index) => {
                                    const isSelected = polygon.id === selectedPolygonId;
                                    const isStartVertex = isSelected &&
                                    ...
                                    return (
                                        <circle
                                            ...
                                            onMouseEnter={() => setHoveredVertex({ polygonId: polygon.id, vertexIndex: index })}
                                            onMouseLeave={() => setHoveredVertex(null)}
                                            onClick={(e) => {
                                                // Stop propagation to prevent the canvas from handling this event
                                                e.stopPropagation();

                                                console.log(`[CanvasV2] Vertex clicked in AddPoints mode: polygon=${polygon.id}, vertex=${index}`);
                                                ...
                                                try {
                                                    ...
                                                    if (!interactionState.isAddingPoints) {
                                                        ...
                                                        if (interactionState.addPointStartVertex.vertexIndex !== index ||
                    ...
                    {editMode === EditMode.AddPoints && selectedPolygonId && (
                        <>
                            ...
                            {/* Render vertices for each temporary point */}
                            {interactionState?.isAddingPoints && tempPoints.map((point, index) => (
                                <circle
                            ...
                            {interactionState?.isAddingPoints && interactionState?.addPointStartVertex &&
                             segmentationData?.polygons && cursorPosition && (
                                (() => {
                                    const selectedPolygon = segmentationData.polygons.find(p => p.id === selectedPolygonId);
