...
describe('CanvasV2 Component', () => {
  ...
  it('handles vertex click with shift in edit vertices mode', () => {
    ...
    expect(mockProps.setInteractionState).toHaveBeenCalledWith(expect.objectContaining({
      isAddingPoints: true,
      addPointStartVertex: {
        polygonId: 'polygon-1',
        vertexIndex: 0
  ...
  it('handles vertex click in add points mode to start adding points', () => {
    render(
      <CanvasV2 
        {...mockProps} 
        editMode={EditMode.AddPoints}
        interactionState={{ isAddingPoints: false }}
      />
    );
  ...
  it('renders different states in create polygon mode based on temp points', () => {
    ...
    rerender(
      <CanvasV2 
        {...mockProps} 
        editMode={EditMode.CreatePolygon}
        tempPoints={[{ x: 100, y: 100 }, { x: 200, y: 100 }]}
      />
    );
    ...
    rerender(
      <CanvasV2 
        {...mockProps} 
        editMode={EditMode.CreatePolygon}
        tempPoints={[{ x: 100, y: 100 }, { x: 200, y: 100 }, { x: 200, y: 200 }]}
      />
    );
    
    // Should show instruction about closing the polygon
    expect(screen.getByText('3. Continue adding points or click near the first point to close the polygon')).toBeInTheDocument();
  });
