...
const CursorLineConnector = ({
  ...
  const startPoint = tempPoints.length > 0 ? tempPoints[tempPoints.length - 1] : polygonPoints[selectedVertexIndex];

  // Koncový bod spojnice - buď bod pod kurzorem, nebo aktuální pozice kurzoru
  const isHoveringEndpoint =
    hoveredSegment.segmentIndex !== null &&
    hoveredSegment.segmentIndex !== selectedVertexIndex &&
    hoveredSegment.projectedPoint;

  const endPoint = isHoveringEndpoint ? hoveredSegment.projectedPoint! : cursorPosition || startPoint;
