...
export const usePointOperations = (
  ...
  const updatePolygonPoints = useCallback((polygonId: string, newPoints: Point[]): boolean => {
   ...
   */
  const addPoint = useCallback((polygonId: string, point: Point): boolean => {
    if (!segmentation) return false;
   ...
   */
  const removePoint = useCallback((polygonId: string, pointIndex: number): boolean => {
    if (!segmentation) return false;
   ...
   */
  const duplicatePoint = useCallback((polygonId: string, pointIndex: number): boolean => {
    if (!segmentation) return false;
