...
const pointLineSegmentDistanceSquared = (
  p: Point,
  a: Point,
  b: Point,
): { distanceSq: number; t: number; closestPoint: Point } => {
...
export const useGeometryUtils = () => {
  ...
  const getPointToSegmentDistance = useCallback(
    (point: Point, segStart: Point, segEnd: Point): { distance: number; closestPoint: Point; t: number } => {
    ...
    (
      ...
      if (closestSegmentIndex !== -1) {
        return {
          ...
          closestPointOnSegment: overallClosestPoint,
        };
      }
