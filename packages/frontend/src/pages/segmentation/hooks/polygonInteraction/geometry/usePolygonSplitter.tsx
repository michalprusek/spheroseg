...
interface ValidationResult {
  ...
  intersections: Array<{ point: Point; segmentIndex: number; t: number }> | null;
  message?: string;
}
...
export const usePolygonSplitter = (
  ...
  const validateAndFindIntersections = useCallback((
    ...
    if (!segmentation) {
      return { isValid: false, polygon: null, intersections: null, message: "Segmentace není k dispozici." };
    }

    const polygon = segmentation.polygons.find(p => p.id === polygonId);
    if (!polygon) {
      return { isValid: false, polygon: null, intersections: null, message: "Polygon nebyl nalezen." };
    }

    console.log(`[POLYGON SPLITTER] Validating slice line from (${startPoint.x.toFixed(2)}, ${startPoint.y.toFixed(2)}) to (${endPoint.x.toFixed(2)}, ${endPoint.y.toFixed(2)}) for polygon ${polygonId}`);
    ...
    if (!isValid) {
      ...
      return { isValid: false, polygon: polygon, intersections: null, message };
    }

    console.log(`[POLYGON SPLITTER] Validation successful, found ${intersections.length} intersections`);
    intersections.forEach((intersection, index) => {
      console.log(`[POLYGON SPLITTER] Intersection ${index + 1}: point (${intersection.point.x.toFixed(2)}, ${intersection.point.y.toFixed(2)}), segment ${intersection.segmentIndex}, t=${intersection.t.toFixed(4)}`);
    });
  ...
  const splitPolygon = useCallback((
    ...
    // Update the polygon
    const updatedPolygons = segmentation!.polygons.map(p => { // segmentation is checked in helper
      if (p.id === polygonId) {
        return {
          ...p,
  ...
  const splitIntoTwoPolygons = useCallback((
    ...
    try {
      ...
      const result = polygonOperations.splitPolygon(
        segmentation,
        polygonId,
        [startPoint, endPoint]
      );
