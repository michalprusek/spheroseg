...
import i18n from 'i18next'; // Import the already initialized i18next instance from i18n.ts
...
// Don't initialize i18n here - it's already initialized in src/i18n.ts
// This prevents the "i18next: initialized" warning
...
export const LanguageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const updateLanguagePreference = useCallback(async (userId: string, lang: Language) => {
    ...
    try {
      ...
      if (testResponse.status === 200) {
        await apiClient.put('/api/users/me', { preferred_language: lang });
  ...
  const detectBrowserLanguage = useCallback((): Language => {
    try {
      ...
      console.log(
        `Neither full (${fullBrowserLanguage}) nor base (${baseLanguage}) language is supported, using default: en`,
      );
      return 'en';
    } catch (error) {
  ...
  const setLanguage = useCallback(
      ...
      i18n
        ...
        .then(() => {
          ...
          if (auth.user && auth.user.id) {
            console.log(
              `[LanguageContext] Asynchronously updating backend language preference for user ${auth.user.id} to ${newLanguage}`,
            );
            apiClient
              .put(`/api/users/me`, { preferred_language: newLanguage }) // Fix: use preferred_language field
              .catch((err) => {
    ...
    [availableLanguages, language, auth.user],
  ); // Changed dependency from auth.isAuthenticated, auth.user?.id to auth.user
  ...
  const t = useCallback(
    ...
    [isInitialized],
  ); // Dependency is only on initialization state
