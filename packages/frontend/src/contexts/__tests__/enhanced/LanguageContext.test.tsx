      ...
      <div data-testid="available-languages">
        ...
        <ul>
          {availableLanguages.map(lang => (
            <li key={lang} data-testid={`lang-${lang}`}>{lang}</li>
          ))}
        </ul>
      </div>
      ...
      <div data-testid="language-switcher">
        ...
        <div>
          ...
          <button 
            ...
            onClick={() => setLanguage('invalid' as Language)}
          >
...
describe('LanguageContext (Enhanced)', () => {
  ...
  it('should handle user login/logout events and language preferences', async () => {
    ...
    require('@/lib/apiClient').__setMockResponse(
      'get', 
      '/users/me', 
      { status: 200, data: { preferred_language: 'fr' } }
    );
  ...
  it('should handle language changes with API updates', async () => {
    ...
    expect(require('@/lib/apiClient').default.put).toHaveBeenCalledWith(
      '/users/me', 
  ...
  it('should handle API errors gracefully when updating language preference', async () => {
    ...
    vi.mocked(require('@/lib/apiClient').default.put).mockRejectedValueOnce(
