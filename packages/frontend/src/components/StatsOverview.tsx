  ...
  return (
    <Card className="transition-all duration-300 hover:shadow-md dark:bg-gray-800 dark:border-gray-700">
      ...
      <CardContent>
        ...
        {descriptionKey && (
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t(descriptionKey, descriptionOptions)}</p>
        )}
        {trend && (
          <div
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <StatCard
        ...
        icon={<FolderKanban />}
        trend={
          stats.projectsComparison
            ? {
                value: stats.projectsComparison > 0 ? `+${stats.projectsComparison}` : String(stats.projectsComparison),
                isPositive: stats.projectsComparison > 0,
              }
            : undefined
        }
      />
      <StatCard
        ...
        icon={<Image />}
        trend={
          stats.imagesComparison
            ? {
                value: stats.imagesComparison > 0 ? `+${stats.imagesComparison}%` : String(stats.imagesComparison),
                isPositive: stats.imagesComparison > 0,
              }
            : undefined
        }
      />
      <StatCard
        ...
        icon={<CheckCircle2 />}
        trend={
          stats.completedComparison
            ? {
                value:
                  stats.completedComparison > 0 ? `+${stats.completedComparison}` : String(stats.completedComparison),
                isPositive: stats.completedComparison > 0,
              }
            : undefined
        }
      />
      <StatCard
        ...
        icon={<Microscope />}
        trend={
          stats.todayComparison
            ? {
                value: stats.todayComparison > 0 ? `+${stats.todayComparison}` : String(stats.todayComparison),
                isPositive: stats.todayComparison > 0,
              }
            : undefined
        }
      />
