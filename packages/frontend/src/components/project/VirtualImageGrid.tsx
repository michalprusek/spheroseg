...
export const VirtualImageGrid = memo(({
  ...
  const renderRow = useCallback((row: ProjectImage[], rowIndex: number) => {
    if (viewMode === 'list') {
  ...
  const getRowKey = useCallback((row: ProjectImage[], index: number) => {
...
export function useVirtualImageGrid(initialColumns: number = 4) {
  ...
  const updateColumns = useCallback((containerWidth: number) => {
