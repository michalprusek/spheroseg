...
const ProjectCard: React.FC<ProjectCardProps> = memo(({ 
  ...
  // Memoized handlers
  const handleDelete = useCallback((deletedProjectId: string) => {
    if (!project.id) {
  ...
  const handleDuplicateSuccess = useCallback((newProject: { id: string; name: string }) => {
  ...
  const projectTitle = useMemo(() => project.name || project.title || 'Untitled Project', [project.name, project.title]);
    ...
    if (project.is_owner === false) {
      return (
        <Badge variant="secondary" className="ml-auto">
          {t('project.shared')} 
          {project.permission && (
            <span className="ml-1 text-xs">({project.permission})</span>
          )}
        </Badge>
      );
    }
