  ...
  return (
    <div className="mb-6 p-6 border rounded-lg bg-card text-card-foreground shadow-sm">
      <div className="flex justify-between items-center mb-4">
        ...
        <Button variant="outline" size="sm" onClick={onCancel} className="border border-gray-300 dark:border-gray-600 rounded-md shadow-sm">
          {t('common.cancel')}
        </Button>
      </div>

      <ImageUploader projectId={projectId} onUploadComplete={(_projId, uploadedImages) => onUploadComplete(uploadedImages)} />
    </div>
  );
};
