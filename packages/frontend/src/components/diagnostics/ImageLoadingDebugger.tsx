 ...
 */
const ImageLoadingDebugger: React.FC<ImageLoadingDebuggerProps> = ({
  imageUrl,
  projectId,
  imageId
}) => {
  ...
  useEffect(() => {
    const testUrls: Record<string, string> = {
      ...
      'Project-specific Backend URL': projectId ? `http://localhost:5001/uploads/${projectId}/${imageUrl.split('/').pop()}` : '',
      'Docker Network URL': `http://cellseg-backend:5000/uploads/${imageUrl.split('/').pop()}`,
      'Project-specific Docker Network URL': projectId ? `http://cellseg-backend:5000/uploads/${projectId}/${imageUrl.split('/').pop()}` : '',
    };

    // Filter out empty URLs
    const filteredUrls = Object.fromEntries(
      Object.entries(testUrls).filter(([_, url]) => url !== '')
    );
          ...
          <ul className="space-y-1">
            {Object.entries(results).map(([name, success]) => (
              <li key={name} className={`flex items-center ${success ? 'text-green-600' : 'text-red-600'}`}>
                <span className={`inline-block w-2 h-2 rounded-full mr-2 ${success ? 'bg-green-600' : 'bg-red-600'}`}></span>
