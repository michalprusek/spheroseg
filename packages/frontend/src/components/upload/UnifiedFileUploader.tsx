  ...
  const defaultFileItemRenderer = (file: any) => (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Progress bar */}
        {file.status === UploadStatus.UPLOADING && <Progress value={file.progress} className="h-1 mt-2" />}
      </div>

      {/* Actions */}
      <div className="flex items-center gap-1">
        {file.status === UploadStatus.ERROR && (
          <Button variant="ghost" size="icon" onClick={() => fileActions.retry(file.id)} title="Retry upload">
            <RefreshCw className="h-4 w-4" />
          </Button>
        )}

        {file.status !== UploadStatus.UPLOADING && (
          <Button variant="ghost" size="icon" onClick={() => fileActions.remove(file.id)} title="Remove file">
            <X className="h-4 w-4" />
          </Button>
        )}
      </div>
        ...
        <Card className="p-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">{formatFileSize(stats.totalSize)}</span>
            ...
            {showActions && (
              <div className="flex items-center gap-2">
                {stats.failed > 0 && (
                  <Button variant="outline" size="sm" onClick={retryFailed} disabled={isUploading}>
                ...
                {stats.pending > 0 && !isUploading && (
                  <Button variant="default" size="sm" onClick={() => uploadFiles()}>
          ...
          <div className="space-y-2 overflow-y-auto" style={{ maxHeight }}>
            {files.map((file) => (renderFileItem ? renderFileItem(file, fileActions) : defaultFileItemRenderer(file)))}
          </div>
        </Card>
      )}
    </div>
