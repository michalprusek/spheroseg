...
jest.mock('fs', () => ({
  ...
  createReadStream: jest.fn().mockImplementation(filePath => { 
    ...
    const stream = {
      ...
      emit: jest.fn((event, ...args) => { // Helper to call listeners
         if (stream.listeners && stream.listeners[event]) {
            stream.listeners[event].forEach(listener => listener(...args));
         }
      }),
...
jest.mock('multer', () => {
  ...
  const multerInstance = {
    // Mock the .array() method (adjust if using .single() or .fields())
    array: (fieldName: string, maxCount?: number) => {
      ...
      return (req: any, res: any, next: () => void) => {
        ...
        if (req.headers['content-type']?.includes('multipart/form-data')) {
            ...
            // The actual file data is handled by supertest, 
            // but the route handler expects req.files to be populated.
            // We need to mock the structure multer provides.
            // Assuming diskStorage is used by the actual route:
            req.files = [
              {
                fieldname: fieldName,
              ...
              // Add more mock files if the test attaches multiple files
            ];
        } else {
    ...
    // Add mocks for single(), fields() if needed
    // Mock storage engines if directly accessed (usually not needed for route tests)
    diskStorage: jest.fn(() => ({ /* Mock storage object if needed */ })),
    memoryStorage: jest.fn(() => ({ /* Mock storage object if needed */ }))
  };

  // The main export of 'multer' is a function that returns the instance
  const multerFn = () => multerInstance;
  // Assign static methods like diskStorage to the main function object
...
describe('Image Routes', () => {
  // Import app directly now as mocks are handled in setup
  ...
  describe('DELETE /api/images/:id', () => {
    it('should delete an image and return 204', async () => {
        ...
        // Verify fs.unlinkSync was called by the route handler (mocked)
        // Note: The paths here depend on the mock DB returning these paths
        expect(fs.unlinkSync).toHaveBeenCalledWith('/fake/storage/path'); 
        expect(fs.unlinkSync).toHaveBeenCalledWith('/fake/thumb/path'); 
    });
